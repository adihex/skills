...
class HerdrAdapter:
    ...
    def launch(self, *, run_id: str, label: str, cwd: str, worktree: str, branch: str,
        ...
        command = ["agent", "start", label, "--cwd", os.path.abspath(cwd), "--workspace", workspace_id,
                   "--", self.pi_command, "-e", self.extension, "--model", model]
        if thinking:
            command += ["--thinking", thinking]
        command += ["--name", label, "@" + os.path.abspath(brief_file)]
        ...
        manifest = {
            ...
            "setup_duration_seconds": round(time.monotonic() - started, 3),
    ...
    def status(self, manifest: dict) -> dict:
        ...
        return {"native_state": state, "pane": pane, "manifest_state": manifest.get("state"),
